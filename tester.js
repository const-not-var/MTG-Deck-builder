...
function addCard(card) {  
    ...
    // WORKING ON COUNTER
    //     let table = document.getElementById("deckTableId");
    // console.log(tableCardNum);
    // let totalRowCount = table.rows.length;
    // let tbodyRowCount = table.rows.length;
    ...
    // }
        ...
        if (id === "apiCard") {
            let cardRow = '<tr id="xCard" class="card-name card-values">'
                cardRow += '<td class="card-values" id="tableNumber">'+ cardCounter() + '</td>'
                cardRow += `<td class="card-values" id="tableCardName" data-bs-container="body" data-bs-toggle="popover" data-bs-placement="left" data-bs-trigger="hover" data-bs-delay=\'{"show":500,"hide":150}\' data-bs-html="true" data-bs-content=\'<img id="cardImg" src="${data.cards[0].imageUrl}">\'>` + cardName + '</td>'
                cardRow += '<td class="card-values" id="cardDateAdded">' + currentDate + '</td>'
                cardRow += '<td><button type="submit" class="x-btn card-values"><img src="assets/photos/delete.png"' + 'onClick="delCard(this)"' + ' alt="A red X" class="x-icon"></button></td>'
                cardRow += '</tr>';
